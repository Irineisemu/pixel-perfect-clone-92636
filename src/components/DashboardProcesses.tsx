      ...
      <section>
        ...
        {processes.length === 0 ? (
          <div className="rounded-xl border border-zinc-200 bg-white p-10 text-center">
            ...
            {lawyers.length === 0 ? (
              <>
                <p className="text-sm text-zinc-600 mb-3">Nenhum advogado cadastrado ainda.</p>
                <Link
                  to="/alvos"
              ...
              <p className="text-sm text-zinc-600">
                A busca de processos está em andamento. Esta página atualiza sozinha.
              </p>
            ) : (
              <p className="text-sm text-zinc-600">
                Nenhum processo vinculado ainda. Use “Buscar processos” no card do advogado.
              </p>
            )}
          </div>
        ) : (
          <div className="rounded-xl border border-zinc-200 bg-white divide-y divide-zinc-100">
            {processes.map((p: any) => (
              <div key={p.id + p.target.id} className="p-4 flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <div className="font-mono text-[13px] text-zinc-900 truncate">{p.processNumber}</div>
                  <div className="mt-1 text-[12px] text-zinc-600">
                    {p.target.name}
                    {p.classCode && <span className="text-zinc-400"> · classe {p.classCode}</span>}
                  </div>
                  {p.lastSyncedAt && (
                    <div className="mt-0.5 text-[11px] text-zinc-400">
                      Sincronizado em {new Date(p.lastSyncedAt).toLocaleString("pt-BR")}
                    </div>
                  )}
                </div>
                <div className="flex flex-col items-end gap-1 flex-shrink-0">
                  <span className="text-[11px] uppercase tracking-wide text-zinc-500">{p.tribunal}</span>
                  {p.matchedVia === "oab" && p.matchedValue && (
                    <span className="text-[11px] text-zinc-500">OAB {formatOABDisplay(p.matchedValue)}</span>
                  )}
                </div>
              </div>
            ))}
